...
public:
	static void Init()
	{
		texture = loadTexture(PATH("Media/Images/bug.png"));
	}
public:
	float stage = rand() % 360;
	Sprite* sprite;
public:
	Enemy(glm::vec2 pos_)
	{
		...
		{
			sprite->pos = pos_;
			sprite->size = Sprite::vec2(80.0f, 50.0f);
...
public:
	static void Init()
	{
		texture = loadTexture(PATH("Media/Images/Tiles1_D.png"));
	}
public:
	Sprite* sprite;
public:
	Bullet(glm::vec2 pos_)
	{
		bullets.push_back(this);
		sprite = new Sprite;
		{
			sprite->pos = pos_;
			sprite->size = Sprite::vec2(10.0f, 100.0f);
			sprite->texture = texture;
		}
	}
	~Bullet()
...
public:
	bool Loop()
	{
		...
		{
			auto d = glm::distance(sprite->pos, enemy->sprite->pos);
			if(d < 40.0f)
			{
				delete enemy;
				return false;
		...
		if(sprite->pos.y > 700.0f) return false;
...
public:
	Player()
	{
		...
		auto image = loadTexture(PATH("Media/Images/image.png"));

		sprite = new Sprite();
		{
			sprite->pos = vec2(400.0f, 50.0f);
			sprite->size = vec2(50.0f);
...
public:
	void Loop()
	{
		if(reload <= 0)
		{
			...
			{
				reload = 10;
				auto bullet = new Bullet(sprite->pos);
			}
		}
		else
		{
		...
		{
			sprite->pos.x += 5.0f;
			if(sprite->pos.x > 800.0f) sprite->pos.x = 800.0f;
		}
		if(GetAsyncKeyState(VK_LEFT))
		{
			sprite->pos.x -= 5.0f;
			if(sprite->pos.x < 0.0f) sprite->pos.x = 0.0f;
		}
	}
};
...
void func()
{
	...
	for(int i = 0; i < 7; ++i)
	{
		auto enemy = new Enemy(glm::vec2(50 + 100 * i, 500));
	}
